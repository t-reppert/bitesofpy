...
class DB:
    ...
    def create(
        ...
        if primary_key not in schema_dict.keys():
        ...
        for idx, s in enumerate(schema):
            if idx == 0:
                ...
                comma = ","
            if s[0] == primary_key:
                schema_str += f" {comma}{s[0]} {s[1].name} primary key"
            else:
                schema_str += f" {comma}{s[0]} {s[1].name}"
        try:
            self.cursor.execute(f"CREATE TABLE {table} ({schema_str})")
    ...
    def insert(self, table: str, values: List[Tuple]):
        ...
        for record in values:
            if len(schema) != len(record):
                raise SchemaError(f"Table {table} expects items with {len(schema)} values.")
            for idx, value in enumerate(record):
                if type(value) != schema[idx][1].value:
                    raise SchemaError(f"Column {schema[idx][0]} expects values of type {schema[idx][1].value.__name__}.")
            self.cursor.execute(f"INSERT INTO {table} VALUES {record}")
    ...
    def select(
        ...
        if len(target) == 2:
            sql = f" SELECT {columns_str} FROM {table} WHERE {target[0]} = ? "
            return self.cursor.execute(sql, (target[1],)).fetchall()
        elif len(target) == 3:
            sql = f"SELECT {columns_str} FROM {table} WHERE {target[0]} {target[1]} ?"
            return self.cursor.execute(sql, (target[2],)).fetchall()
    ...
    def update(self, table: str, new_value: Tuple[str, Any], target: Tuple[str, Any]):
        ...
        """
        new_field = new_value[0]
        target_field = target[0]
        sql = f''' UPDATE {table}
                   SET {new_field} = ?
                   WHERE {target_field} = ?
        '''
        task = (new_value[1], target[1])
