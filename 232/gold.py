...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    gold_price_list = gold_prices.split()
    gold_price_tuples = [ (int(x.split(',')[0][0:4]),float(x.split(',')[1])) for x in gold_price_list ]
    prev_price = gold_price_tuples[0][1]
    diffs = []
    for p in gold_price_tuples[1:]:
        diffs.append((p[0], p[1]-prev_price))
        prev_price = p[1]
    return (min(diffs,key=lambda x:x[1])[0], max(diffs,key=lambda x:x[1])[0])
