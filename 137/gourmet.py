...
WINES = { 'all': RED_WINES+WHITE_WINES+SPARKLING_WINES,
          'red': RED_WINES,
          'white': WHITE_WINES,
          'sparkling': SPARKLING_WINES }

def get_score(wine, cheese):
    wine_count = Counter(wine.lower())
    cheese_count = Counter(cheese.lower())
    counted = wine_count & cheese_count
    return sum(counted.values()) / (1 + (len(wine)-len(cheese))**2)
...
def best_match_per_wine(wine_type="all"):
    ...
    if wine_type not in WINES:
        raise ValueError
    wine_list = WINES[wine_type]
    wine_cheese_scores = []
    for wine in wine_list:
        for cheese in CHEESES:
            wine_cheese_scores.append((wine, cheese, get_score(wine, cheese)))
    return max(wine_cheese_scores,key=operator.itemgetter(2))
...
def match_wine_5cheeses():
    ...
    """
    wines = sorted(WINES["all"])
    matched = []
    for wine in wines:
        cheese_scores = defaultdict(int)
        for cheese in CHEESES:
            cheese_scores[cheese] = get_score(wine, cheese)
        matched.append( (wine, [ k for k,v in sorted(cheese_scores.items(), key=lambda kv:(-kv[1],kv[0]) ) ][:5]))
