...
def get_food_most_calories(df=df):
    ...
    return df.sort_values('Calories').iloc[-1].Item
...
def get_bodybuilder_friendly_foods(df=df, excl_drinks=False):
    ...
    df = df[df['Calories']!=0]
    df['ProteinCalRatio'] = df['Protein']/df['Calories']
    return df[['Item','ProteinCalRatio']].sort_values('ProteinCalRatio',ascending=False).iloc[:5]['Item'].tolist()