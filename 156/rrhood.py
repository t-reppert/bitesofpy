    ...
    for i, line in enumerate(text.splitlines(),start=0):
        ...
        for character in characters:
            if type(character) == tuple:
                # tuple - multiple synonyms - loop over each
                for synonym in character:
                    if synonym.lower() in line:
                        character_index[character[0].lower()].append(i)
                        break
            else:
                # single character - no synonyms
                if character.lower() in line:
                    character_index[character.lower()].append(i)
