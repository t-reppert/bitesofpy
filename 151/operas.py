...
def operas_both_at_premiere(guest, composer):
    ...
    for opera in operas:
        if guest not in composers or composer not in composers:
            raise ValueError
        if opera.author != guest and opera.author == composer:
            if _get_date(opera.date) > _get_date(composers[composer].born) and \
               _get_date(opera.date) > _get_date(composers[guest].born) and \
               _get_date(opera.date) < _get_date(composers[composer].died) and \
               _get_date(opera.date) < _get_date(composers[guest].died):
               yield opera.play
