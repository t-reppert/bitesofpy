...
def sysinfo_scrape(output):
    ...
    import re
    regex = re.compile(r'(\w+\@\w+|OS:\s.*|Kernel.*|Uptime:.*|Packages:.*|Shell:.*|Resolution:.*|DE:.*|WM:.*|WM Theme:.*|GTK Theme:.*|Icon Theme:.*|Font:.*|GPU:.*|CPU:.*|RAM:.*)',re.M)
    new_output = regex.findall(output)
    data = {}
    data['Name'] = new_output[0]
    for line in new_output[1:]:
        label, info = line.split(':')
        data[label] = info.strip()
    return data
