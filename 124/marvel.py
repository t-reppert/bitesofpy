...
def max_and_min_years_new_characters():
    """Get the year with most and least new characters introduced respectively,
       ...
       (max_year, min_year)"""
    year_counts = Counter([ x.year for x in data if x.year != ''])
    max_year = year_counts.most_common()[0]
    min_year = year_counts.most_common()[-1]
    return (max_year[0], min_year[0])


def percentage_female():
    """Get the percentage of female characters as percentage of all characters, rounded to 2 digits"""
    female_chars = len([ x.sex for x in data if x.sex == 'Female Characters'])
    return round((female_chars/len(data))*100,2)