...
def filter_killed_mutants(mutpy_output: list = None) -> list:
    ...
    mutpy_output = list(reversed(mutpy_output))
    new_mutpy = []
    check = False
    dash_line = 0
    for line in mutpy_output:
        if '] killed' in line or '] incompetent' in line:
            check = True
            dash_line = 0
            new_mutpy.append(line)
            continue
        if check == True and '----------' in line:
            dash_line += 1
            if dash_line == 2:
                check = False
                continue
        if check != True:
            new_mutpy.append(line)
    new_mutpy = list(reversed(new_mutpy))
    return new_mutpy
